...
<head>
    <meta charset="utf-8">
    <title>TrustNode Database Security Assessment Report</title>
    <style>
...
<body>
    ...
    <div class="cover-page page">
        <div class="cover-header">
            ...
            <div class="cover-title">Database Security Assessment</div>
        ...
        <div class="cover-details-container">
            <div class="cover-details">
                <table>
                    ...
                    <tr>
                        <th>Engine Used</th>
                        <td>{{ strtoupper(str_replace('_', ' ', $scan->engine->value ?? 'Database Scanner')) }}</td>
                    </tr>
                    <tr>
    ...
    <div class="page">
        <div class="report-header">
            ...
            <div style="font-size: 18px; color: #4b5563; margin-top: 5px;">Database Security Assessment Report</div>
        </div>
        ...
        <table>
            ...
            <tr>
                ...
                <td>{{ $checkStats['total'] ?? 0 }}</td>
                <th>Passed Checks</th>
                <td>{{ $checkStats['passed'] ?? 0 }}</td>
            </tr>
            <tr>
                <th>Total Findings</th>
                <td>{{ count($findings) }}</td>
        ...
        <p class="section-desc">The following security issues were detected by the TrustNode database security scanner.</p>

        @if (count($findings) === 0)
            <p style="text-align: center; color: green; font-weight: bold; margin: 40px 0; padding: 20px; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px;">No vulnerabilities detected in this database.</p>
        @else
            @foreach ($findings as $f)
                <div class="finding-card">
                    ...
                    <p><strong>Target:</strong> <code>{{ $f->url }}</code></p>
        ...
        <p>This assessment was performed using active connection profiling and configuration checks. The scanner queries the database for known weak configurations, missing access controls, and risky parameter values. This scanner does not exploit vulnerabilities, and credential evaluation is limited to the privileges granted to the provided connection account.</p>
